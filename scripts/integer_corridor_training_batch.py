"""Ragged, CPU-only batching helpers for offline corridor-policy training.
...
def prepare_records(records: Sequence[dict[str, Any]]) -> list[PreparedRecord]:
    """Cache normalized geometry, context, and candidate indexing once."""

    return [PreparedRecord.from_record(record) for record in records]
...
def _batched_features(policy: CorridorPolicy, prepared: Sequence[PreparedRecord]) -> tuple[torch.Tensor, tuple[int, ...]]:
    ...
    for record in prepared:
        for start, end in record.plane_offsets:
            start += plane_offset
            end += plane_offset
            corridor = hidden[start:end]
            encoded.append(torch.cat((corridor.mean(dim=0), corridor.max(dim=0).values,
                                      hidden.new_tensor([float(end - start)]))))
        ...
        plane_offset += record.plane_values.shape[0]
        corridor_offset += len(record.plane_offsets)

    candidates = torch.cat(candidate_indices, dim=0)
    corridor_features = torch.stack(encoded).index_select(0, candidates.reshape(-1))
