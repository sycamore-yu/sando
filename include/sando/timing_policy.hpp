...
class TimingPolicy {
 public:
  ...
  int proposalCount() const noexcept { return proposal_count_; }
 ...
 private:
  ...
  void load(const std::string& path) {
    ...
    if ((schema != 1 && schema != 2) || !j.contains("type") || !j.contains("feature_spec") ||
    ...
    nfe_=j.value("nfe",1); setNfe(nfe_);
    if(schema==1){
      if(!j.contains("logfactor_bounds") || j.at("logfactor_bounds") != nlohmann::json::array({0.0,std::log(5.0)})) throw std::invalid_argument("invalid timing log bounds");
      if(!j.contains("latent_quantiles") || j.at("latent_quantiles") != nlohmann::json::array({-1.0,0.0,1.0})) throw std::invalid_argument("invalid timing quantiles");
      log_min_=0; log_max_=std::log(5.0); quantiles_={-1.0,0.0,1.0}; proposal_count_=3;
    } else {
      if(j.value("output_mode","")!="single"||!j.contains("proposal_count")||j.at("proposal_count")!=1) throw std::invalid_argument("invalid timing policy schema");
      if(!j.contains("logfactor_bounds") || j.at("logfactor_bounds") != nlohmann::json::array({0.0,std::log(2.5)})) throw std::invalid_argument("invalid timing log bounds");
      if(!j.contains("latent_quantiles") || j.at("latent_quantiles") != nlohmann::json::array({0.0})) throw std::invalid_argument("invalid timing quantiles");
      log_min_=0; log_max_=std::log(2.5); quantiles_={0.0}; proposal_count_=1;
    }
    residual_std_=j.value("residual_std",0.0); if(!std::isfinite(residual_std_)||residual_std_<0)throw std::invalid_argument("invalid timing residual"); valid_=true;
  }
  std::string path_, type_; bool valid_{false}; int nfe_{1}; int proposal_count_{3}; double residual_std_{0}; double log_min_{0},log_max_{std::log(5.0)}; double normalizer_mean_[17]{},normalizer_std_[17]{}; std::vector<Layer> layers_; std::vector<double> quantiles_{-1.0,0.0,1.0};
};
}
